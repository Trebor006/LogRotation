...
for folder in "${folders[@]}"; do
    ...
    find "$local_folder" -type f -newermt "$(date -d "-1 month" +%Y-%m-%d)" | while read file; do
        ...
        if [ $? -eq 0 ]; then
            echo "\e[35mMONTHLY:: Archivo $file transferido correctamente a $remote_folder. Eliminando...\e[0m"
            rm "$file"
        else
            echo "\e[35mMONTHLY:: Error al transferir $file. No se eliminará.\e[0m"
        fi
    done
done

echo "\e[35mMONTHLY:: Transferencia y limpieza de archivos completada.\e[0m"
